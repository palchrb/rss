...
class RSSBot(Plugin):
    ...
    async def _send(self, feed: Feed, entry: Entry, sub: Subscription) -> EventID | None:
        if sub.title_exclude_filter and re.search(sub.title_exclude_filter, entry.title):
            return None
    ...
    async def set_filter(self, evt: MessageEvent, feed_id: int, title_exclude_filter: str) -> None:
        if evt.sender in self.config["admins"]:
            pass
        elif not self.config["allow_filter"]:
            await evt.reply("This bot does not allow non-admin users to set title filters")
            return
        elif not await self.can_manage(evt):
            return
